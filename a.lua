shared.azov = {
    ...
    ["silentaim"] = {
        ...
        ["point"] = 'closest point', -- closest point, closest part, part name, part table
...
if not LPH_OBFUSCATED then
    ...
    do
        ...
        function Script:UpdateLabels()
            ...
            if not BrandFrame then
                ...
                BrandFrame.AnchorPoint = Vector2.new(0.5, 0.5)
                BrandFrame.Size = UDim2.new(0, 140, 0, SZ + 6)
                ...
                for i = 1, 5 do
                    ...
                    container.Size                   = UDim2.new(0, 500, 0, SZ + 6)
                    ...
                    feat.Size                   = UDim2.new(0.35, 0, 1, 0)
                    feat.AnchorPoint            = Vector2.new(1, 0.5)
                    feat.Position               = UDim2.fromScale(0.5, 0.5)
                    feat.Font                   = FONT
                    feat.TextSize               = SZ
                    ...
                    tFrame.Size                   = UDim2.new(0.5, 0, 1, 0)
                    tFrame.AnchorPoint            = Vector2.new(0, 0.5)
                    tFrame.Position               = UDim2.fromScale(0.5, 0.5)
            ...
            BrandFrame.Position = UDim2.fromOffset(cx, viewportSize.Y / 2 + 46)
            ...
            for _, e in ipairs(lines) do
                local row = Script.Locals.HudLines[e.i]
                row.container.Visible  = true
                row.container.Position = UDim2.fromOffset(cx, viewportSize.Y / 2 + offsetY)
    ...
    do
        ...
        local function ir_get_target_part(character)
            if shared.azov["dmg_override"]["mode"] == 'full' then
        ...
        local function ir_do_bullet_tp(tool, mode)
            local target = Script.Locals.SilentAimTarget or Script:GetClosestPlayerToCursor(math.huge, math.huge)
            ...
            if mode == 'wallbang' then
                ...
                local to_shooter = (origin_part.Position - target_part.Position).Unit
                tp_cf = CFrame.new(target_part.Position + to_shooter * 0.2)
            elseif mode == 'range' then
                local to_shooter = (origin_part.Position - tHRP.Position).Unit
                tp_cf = CFrame.new(tHRP.Position + to_shooter * 0.2)
            end
            if not tp_cf then return end
            local original_grip = tool.Grip
            tool.Parent = Self.Backpack
            tool.Grip   = ir_cframe_to_offset(origin_part.CFrame, tp_cf)
        ...
        local function ir_wire_tool(tool)
            ...
            ir_ActivatedConn = tool.Activated:Connect(function()
                if not (shared.azov["infinite_range"]["enabled"] or shared.azov["bullet_wallbang"]["enabled"]) then return end
                local target = Script.Locals.SilentAimTarget or Script:GetClosestPlayerToCursor(math.huge, math.huge)
                if not (target and target.Character) then return end
                ...
                if not visible and shared.azov["bullet_wallbang"]["enabled"] then
                    ir_do_bullet_tp(tool, 'wallbang')
                elseif dist > range and shared.azov["infinite_range"]["enabled"] then
                    ir_do_bullet_tp(tool, 'range')
                end
            end)
        end
