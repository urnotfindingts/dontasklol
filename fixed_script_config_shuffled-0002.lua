shared.azov = {
    ["globals"] = {
        ...
        ["target tracer"] = {
            ["enabled"]           = true,
            ["thickness"]         = 0.5,
            ["effective color"]   = Color3.fromRGB(255, 85, 85),   -- on-target / ShouldShoot passing
            ["ineffective color"] = Color3.fromRGB(0, 0, 0), -- tracking but not shooting
        },
        ["exploit tracer"] = {
            ["enabled"]   = true,
            ["thickness"] = 0.5,
    ...
    ["aimbot"] = {
        ["enabled"] = true,
        ["toggle"] = 'C',
        ["point"] = 'Heae', -- closest point, closest part, part name, part table
    ...
    ["silentaim"] = {
        ...
        ["point"] = 'Head', -- closest point, closest part, part name, part table
        ["closest point"] = { ["mode"] = 'advanced', ["scale"] = 0.15 },
        ["hitpart override"] = { ["enabled"] = false, ["toggle"] = 'F', 'head' },
    ...
    ["exploits"] = { -- Da Hood Specific
        ["doubletap"] = { ["enabled"] = false, ["toggle"] = 'E', { "[Revolver]", "[Silencer]" } },
        ["spread modifier"] = {
            ["enabled"] = true,
            ["value"] = 0,
    ...
    ["movement"] = {
        ["enabled"] = true,
        ["speed"] = { ["value"] = 300, ["toggle"] = 'Z', ["enabled"] = true, ["glide"] = false, ["mode"] = 'always' }, -- toggle / hold / always
...
if not LPH_OBFUSCATED then
    ...
    local function InitializeLocals()
        local defaults = {
            ...
            LPH_ENCSTR("IsForcehit"), LPH_ENCSTR("IsBulletTp"), LPH_ENCSTR("ForcehitTarget"), LPH_ENCSTR("BulletTpTarget")
        }
    ...
    do
        ...
        function Script:UpdateLabels()
            ...
            local btCfgL  = shared.azov["exploits"]["bullet tp"]
            local btTypeL = btCfgL["type"] or 'toggle'
            if btCfgL["enabled"] and (btTypeL == 'always' or Script.Locals.IsBulletTp) then
                local tgt   = Script.Locals.BulletTpTarget
                local tname = tgt and ("  (" .. tgt.DisplayName .. ")") or "  (N/A)"
                table.insert(lines, { feat = "Bullet TP", tname = tname })
            end
    ...
    do
        ...
        local function UpdateDrawings()
            ...
            Script.Locals.FieldOfViewTwo.Position = GetAimPosition() + Vector2.new(0, GuiInsetOffsetY)
            Script:UpdateBox()
            Script:UpdateLabels()
            ...
            if ttCfg["enabled"] and ttTarget and ttTarget.Character then
                ...
                if tHRP then
                    ...
                    if onScreen then
                        ...
                        local isMobile = UserInputService.TouchEnabled and not UserInputService.MouseEnabled
                        local fromPt   = isMobile
                            and Vector2.new(aimPos.X, aimPos.Y)
                            or  Vector2.new(aimPos.X, aimPos.Y + GuiInsetOffsetY)
                        local toPt     = Vector2.new(screenPos.X, screenPos.Y)

                        -- Ineffective checks:
                        -- 1. Visibility through walls/fences/cover
                        local visibleToCamera = Script:RayCast(tHRP, Script:GetOrigin('Camera'), {Self.Character, TriggerPart, SilentAimPart})

                        -- 2. Weapon range (only if a tool with Range exists)
                        local tracerTool = Character:FindFirstChildWhichIsA("Tool")
                        local tracerRange = tracerTool and tracerTool:FindFirstChild("Range")
                        local inRange = true
                        if tracerRange then
                            local dist = (Camera.CFrame.Position - tHRP.Position).Magnitude
                            inRange = dist <= tracerRange.Value
                        end

                        -- 3. Within FOV / ShouldShoot
                        local shouldShoot = Script.Locals.IsBoxFocused or
                            (shared.azov["silentaim"]["fov"]["type"] == 'circle' and
                             (fromPt - toPt).Magnitude <= (CurrentFOV or math.huge))

                        local isEffective = visibleToCamera and inRange and shouldShoot
            ...
            -- Exploit tracer (top-center -> forcehit or bullet tp target, only when active)
            local etCfg    = shared.azov["globals"]["exploit tracer"]
            local fhCfgET  = shared.azov["exploits"]["forcehit"]
            local btCfgET  = shared.azov["exploits"]["bullet tp"]
            local fhTypeET = fhCfgET["type"] or 'toggle'
            local btTypeET = btCfgET["type"] or 'toggle'
            local fhActiveET = fhCfgET["enabled"] and (fhTypeET == 'always' or Script.Locals.IsForcehit)
            local btActiveET = btCfgET["enabled"] and (btTypeET == 'always' or Script.Locals.IsBulletTp)
            -- Pick the exploit target: bullet tp target takes priority, then forcehit target
            local etTarget = (btActiveET and Script.Locals.BulletTpTarget)
                          or (fhActiveET and Script.Locals.ForcehitTarget)
                          or nil
            if etCfg["enabled"] and (fhActiveET or btActiveET) and etTarget and etTarget.Character then
        ...
        RBXConnection(UserInputService.InputBegan, function(Input, Processed)
            ...
            -- Bullet TP toggle/hold + target selection
            local btCfg = shared.azov["exploits"]["bullet tp"]
            if btCfg["enabled"] then
                local btType = btCfg["type"] or 'toggle'
                local btKey  = Enum.KeyCode[btCfg["toggle"]:upper()]
                if btType ~= 'always' and Input.KeyCode == btKey then
                    if btType == 'toggle' then
                        Script.Locals.IsBulletTp = not Script.Locals.IsBulletTp
                        if not Script.Locals.IsBulletTp then
                            Script.Locals.BulletTpTarget = nil
                            BulletTpToggle = false
                        end
                    elseif btType == 'hold' then
                        Script.Locals.IsBulletTp = true
                    end
                end
                if btCfg["selection"] == 'target' then
                    local btTgtKey = Enum.KeyCode[btCfg["target toggle"]:upper()]
                    if Input.KeyCode == btTgtKey then
                        BulletTpToggle = not BulletTpToggle
                        Script.Locals.BulletTpTarget = BulletTpToggle
                            and Script:GetClosestPlayerToCursor(math.huge, math.huge) or nil
                    end
                end
            end
        ...
        RBXConnection(UserInputService.InputEnded, function(Input, Processed)
            ...
            local btCfgE = shared.azov["exploits"]["bullet tp"]
            if btCfgE["enabled"] and (btCfgE["type"] or 'toggle') == 'hold' then
                local btKeyE = Enum.KeyCode[btCfgE["toggle"]:upper()]
                if Input.KeyCode == btKeyE then
                    Script.Locals.IsBulletTp = false
                end
            end
        end)
        RBXConnection(RunService.PreRender, LPH_NO_VIRTUALIZE(function()
            ...
            local btCfgPR = shared.azov["exploits"]["bullet tp"]
            if btCfgPR["enabled"] and btCfgPR["selection"] == 'automatic' then
                Script.Locals.BulletTpTarget = Script:GetClosestPlayerToCursor(math.huge, math.huge)
            end
    ...
    do
        local function ir_is_visible(from, tHRP)
            local params = RaycastParams.new()
            params.FilterType = Enum.RaycastFilterType.Exclude
            params.FilterDescendantsInstances = { Self.Character }
            local result = workspace:Raycast(from, tHRP.Position - from, params)
            if not result then return true end
            return result.Instance and result.Instance:IsDescendantOf(tHRP.Parent)
        end

        local function ir_get_target_part(character)
            if shared.azov["exploits"]["forcehit"]["mode"] == 'full' then
        ...
        local function ir_get_range(tool)
            local r = tool and tool:FindFirstChild("Range")
            return r and r.Value or 200
        end

        local function ir_cframe_to_offset(origin, target)
            -- Compute grip offset so that when the tool is equipped,
            -- the handle sits at `target` position/orientation in world space.
            return origin:ToObjectSpace(target)
        end

        local function ir_do_bullet_tp(tool, mode, overrideTarget)
            local target = overrideTarget
                or Script.Locals.BulletTpTarget
                or Script.Locals.SilentAimTarget
                or Script:GetClosestPlayerToCursor(math.huge, math.huge)
            if not (target and target.Character) then return end
            local char        = Self.Character
            local origin_part = char and (char:FindFirstChild("RightHand") or char:FindFirstChild("HumanoidRootPart"))
            local tHRP        = target.Character:FindFirstChild("HumanoidRootPart")
            if not (origin_part and tHRP) then return end

            -- Pick the exact aim point: use silent aim HitPosition when available,
            -- otherwise fall back to the target part position.
            local aim_pos
            local hitPos = Script.Locals.HitPosition
            if hitPos and hitPos ~= Vector3.zero then
                aim_pos = hitPos
            else
                local target_part = ir_get_target_part(target.Character)
                aim_pos = (target_part and target_part.Position) or tHRP.Position
            end

            -- Muzzle position: step toward the shooter from aim_pos just enough
            -- that the bullet originates inside range and travels to aim_pos.
            local to_shooter = (origin_part.Position - aim_pos).Unit
            local tp_pos
            if mode == 'wallbang' then
                -- Place muzzle 0.2 studs in front of the aim point on the shooter side
                tp_pos = aim_pos + to_shooter * 0.2
            elseif mode == 'range' then
                -- Place muzzle just inside the weapon's range so the bullet reaches
                local range = ir_get_range(tool)
                local dist  = (origin_part.Position - aim_pos).Magnitude
                if dist <= range then return end  -- already in range, no TP needed
                -- Step from aim_pos toward shooter by a tiny margin
                tp_pos = aim_pos + to_shooter * 0.2
            end
            if not tp_pos then return end

            -- Safety: if tp_pos == aim_pos the LookAt CFrame would NaN
            if (tp_pos - aim_pos).Magnitude < 0.01 then
                tp_pos = aim_pos + to_shooter * 0.5
            end

            -- redirect_cf: placed at tp_pos, facing toward aim_pos
            -- The grip offset moves the muzzle there so the fired bullet
            -- originates at tp_pos and travels straight into aim_pos.
            local redirect_cf = CFrame.new(tp_pos, aim_pos)

            local original_grip = tool.Grip
            tool.Parent = Self.Backpack
            tool.Grip   = ir_cframe_to_offset(origin_part.CFrame, redirect_cf)
            tool.Parent = Self.Character
            -- One heartbeat is enough for the server to register the grip change
            task.wait()
            tool.Parent = Self.Backpack
        ...
        local ir_ActivatedConn = nil

        local function ir_wire_tool(tool)
            if ir_ActivatedConn then ir_ActivatedConn:Disconnect() end
            ir_ActivatedConn = tool.Activated:Connect(function()
                if not (shared.azov["exploits"]["infinite range"]["enabled"] or shared.azov["exploits"]["bullet tp"]["enabled"]) then return end
                -- Bullet TP uses its own target; infinite range uses silent aim target
                local btCfgW  = shared.azov["exploits"]["bullet tp"]
                local btTypeW = btCfgW["type"] or 'toggle'
                local btActive = btCfgW["enabled"] and (btTypeW == 'always' or Script.Locals.IsBulletTp)
                local irActive = shared.azov["exploits"]["infinite range"]["enabled"]

                if not (btActive or irActive) then return end

                local btTarget = Script.Locals.BulletTpTarget
                    or Script.Locals.SilentAimTarget
                    or Script:GetClosestPlayerToCursor(math.huge, math.huge)
                if not (btTarget and btTarget.Character) then return end

                local myHRP = Self.Character and Self.Character:FindFirstChild("HumanoidRootPart")
                local tHRP  = btTarget.Character:FindFirstChild("HumanoidRootPart")
                if not (myHRP and tHRP) then return end
                local dist    = (myHRP.Position - tHRP.Position).Magnitude
                local range   = ir_get_range(tool)
                local visible = ir_is_visible(myHRP.Position, tHRP)
                if not visible and btActive then
                    ir_do_bullet_tp(tool, 'wallbang', btTarget)
                elseif dist > range and irActive then
                    ir_do_bullet_tp(tool, 'range', btTarget)
                end
            end)
        end

        local ir_ChildAddedConn = nil
        local ir_ChildRemovedConn = nil

        local function ir_handle_character(character)
            if ir_ChildAddedConn   then ir_ChildAddedConn:Disconnect()   end
            if ir_ChildRemovedConn then ir_ChildRemovedConn:Disconnect() end
            local existing = character:FindFirstChildOfClass("Tool")
            if existing then ir_wire_tool(existing) end
            ir_ChildAddedConn = character.ChildAdded:Connect(function(child)
                if child:IsA("Tool") then ir_wire_tool(child) end
            end)
            ir_ChildRemovedConn = character.ChildRemoved:Connect(function(child)
                if child:IsA("Tool") and ir_ActivatedConn then
                    ir_ActivatedConn:Disconnect()
                end
            end)
        end

        ir_handle_character(Self.Character or Self.CharacterAdded:Wait())
        Self.CharacterAdded:Connect(ir_handle_character)
